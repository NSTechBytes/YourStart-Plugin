    ...
    internal class Measure
    {
        ...
        private bool ExtractIconFromExe(string exePath, string savePath)
        {
            try
            {
                ...
                using (Icon icon = Icon.ExtractAssociatedIcon(exePath))
                {
                    if (icon == null)
                    {
                        api.Log(
                            API.LogType.Warning,
                            $"YourStart.dll: No icon found for '{exePath}'."
                        );
                    ...
                    {
                        // Check if the icon is already 64x64 to avoid unnecessary resizing
                        if (originalBitmap.Width == 64 && originalBitmap.Height == 64)
                        {
                            // Save the original bitmap directly
                            originalBitmap.Save(savePath, System.Drawing.Imaging.ImageFormat.Png);
                        }
                        else
                        {
                            // Resize the bitmap to 64x64 while maintaining aspect ratio
                            using (Bitmap resizedBitmap = new Bitmap(64, 64))
                            {
                                using (Graphics graphics = Graphics.FromImage(resizedBitmap))
                                {
                                    ...
                                    graphics.InterpolationMode = System
                                        ...
                                        .InterpolationMode
                                        .HighQualityBicubic;
                                    graphics.SmoothingMode = System
                                        .Drawing
                                        .Drawing2D
                                        .SmoothingMode
                                        .AntiAlias;
                                    graphics.PixelOffsetMode = System
                                        .Drawing
                                        .Drawing2D
                                        .PixelOffsetMode
                                        .HighQuality;
                                ...
                                // Save the resized bitmap as a PNG
                                resizedBitmap.Save(
                                    savePath,
                ...
                api.Log(
                    API.LogType.Notice,
                    $"YourStart.dll: Icon successfully extracted and saved to '{savePath}'."
                );
            ...
            catch (UnauthorizedAccessException ex)
            {
                api.Log(
                    API.LogType.Error,
                    $"YourStart.dll: Access denied when saving icon to '{savePath}': {ex.Message}"
                );
            ...
            catch (ArgumentException ex)
            {
                api.Log(
                    API.LogType.Error,
                    $"YourStart.dll: Invalid file path '{savePath}': {ex.Message}"
                );
            ...
            catch (Exception ex)
            {
                api.Log(
                    API.LogType.Error,
                    $"YourStart.dll: Unexpected error during icon extraction from '{exePath}': {ex.Message}"
                );
